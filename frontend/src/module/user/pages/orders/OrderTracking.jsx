...
export default function OrderTracking() {
  ...
  const handleShare = async () => {
    const restaurantSlug = order?.restaurantSlug || order?.restaurantId?.slug || "";
    const restaurantName = order?.restaurantName || order?.restaurantId?.name || "the restaurant";
    
    // Create share URL - specifically for tracking
    const shareUrl = `${window.location.origin}/user/orders/tracking/${orderId}`;
    const shareText = `Track my order from ${restaurantName} on GrhaPoch!`;

    setShareData({
      title: `Order Tracking - ${restaurantName}`,
      text: shareText,
      url: shareUrl
    });
