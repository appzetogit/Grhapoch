...
export default function RestaurantSignup() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "phone") {
      ...
      // Clear error while typing, only show if it reaches valid length
      if (numericValue.length === 10) {
        setErrors({ ...errors, phone: validatePhone(numericValue) })
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const otpResponse = await restaurantAPI.sendOTP(fullPhone, "register")
      ...
      <div className="w-full lg:w-1/2 h-full flex flex-col">
        ...
        >
          ...
          >
            ...
            <Button
              type="submit"
              className={`mt-2 h-11 w-full text-white text-base font-semibold rounded-md shadow-md transition-colors ${
                !formData.phone || formData.phone.length !== 10 || !formData.name || errors.phone || errors.name || isLoading
                  ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                  : "bg-primary-orange hover:bg-primary-orange/90"
              }`}
              disabled={!formData.phone || formData.phone.length !== 10 || !formData.name || errors.phone || errors.name || isLoading}
            >
              {isLoading ? (
                <>
